...
def add_livro():
    ...
    try:
        ...
        query = text("""
            INSERT INTO Livros (Titulo, ISBN, Ano_publicacao, Quantidade_disponivel, Autor_id, Resumo)
            VALUES (:titulo, :isbn, :ano, :qtd, :autor_id, :resumo)
        """)
        db.execute(query, {
            "titulo": titulo,
            "isbn": isbn,
...
def remover_livro(id_livro):
    ...
    try:
        ...
        emprestimos_ativos = db.execute(text("""
            SELECT COUNT(*) AS total
            FROM Emprestimos
            WHERE Livro_id = :id AND Status_emprestimo IN ('pendente', 'atrasado')
        """), {"id": id_livro}).fetchone()

        if emprestimos_ativos.total > 0:
            flash("Não é possível remover este livro, pois há empréstimos ativos.")
            return redirect(url_for('dashboard'))

        db.execute(text("""
            DELETE FROM Emprestimos
            WHERE Livro_id = :id AND Status_emprestimo = 'devolvido'
        """), {"id": id_livro})
...
def emprestar_livro(id_livro):
    ...
    try:
        livro = db.execute(text("""
            SELECT Quantidade_disponivel FROM Livros WHERE ID_livro = :id
        """), {"id": id_livro}).fetchone()
